...
impl GameState {
    fn reset_game(&mut self) {
        ...
        self.current_state = vec!['_'; self.word_to_guess.len()];
    }
}


fn handle_client(mut stream: TcpStream, word_to_guess: String,game_state: Arc<Mutex<GameState>>,word_found_tx: Arc<Mutex<mpsc::Sender<bool>>>) {
   ...
   while play_again {   
        ...
        if attempts > 0 {
            ...
            word_found_tx.lock().unwrap().send(true).unwrap();
        } else {
            stream.write(b"You lost! The word was ").unwrap();
            stream.write(word_to_guess.as_bytes()).unwrap();
            stream.write(b"\n").unwrap();
        }
        stream.write(b"Do you want to play again? (y/n)\n").unwrap();
...
fn main() {
    ...
    let  word_found_tx = Arc::new(Mutex::new(tx));
    ...
    for stream in listener.incoming() {
        ...
        let word_found_tx = word_found_tx.clone();
        thread::spawn(move || {
            handle_client(stream, word_to_guess,game_state,word_found_tx);
        });
    }
    while let Ok(word_found) = rx.recv() {
        if word_found {
            println!("Le mot a été trouvé par un joueur ! Les autres joueurs ont perdu.");
        }
    }
}
